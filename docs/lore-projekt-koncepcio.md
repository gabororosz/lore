...
**Még nyitott:** az invariáns-robusztusság technikai része (a nem-birtokolt repó elrontott frontmattere elleni védelem), és a brown-field migráció (Confluence/Jira/Slack → repo), amit tudatosan post-MVP-re toltunk.
...
### Az AI ugyanabból a kontextusból dolgozik, mint a fejlesztők

Mivel a dokumentáció és a kód ugyanabban a repositoryban él, az AI egyszerre látja a `src/`, `docs/`, `.github/`, `terraform/` és egyéb tartalmat. Ez sokkal pontosabb elemzést tesz lehetővé, mint amikor a dokumentáció külön rendszerben található.
...
3. **Piszkozat.** A felhasználó folyamatosan szerkeszthet; a web-es alkalmazás automatikusan perzisztálja a változásokat (piszkozat). Ez a szerveren élő, **átmeneti** állapot — úton a Git felé.
...
- amíg valakinél a lock, a többiek a `Publish`-olt (vagy piszkozat-)állapotot olvassák, „X szerkeszti" jelöléssel;
...
- **átvétel:** lejárt vagy elhagyott lockot egy másik felhasználó átvehet — a rendszer figyelmeztet, ha eközben mentetlen piszkozat maradna;
...
A képek automatikusan a repositoryba kerülnek (pl. `/docs/assets/images`). Kép beszúrásakor a Lore feltölti a képet, létrehozza a Markdown-hivatkozást, majd a Publish folyamat részeként commitolja és pusholja.
...
**Fontos korlát — mit ad ez és mit nem:** ez **emberi (dokumentációs) védővonal, nem technikai garancia.** Aki nem olvassa el, vagy programból írja a fájlt, átgázol rajta. A YAML-komment (`#`) ráadásul nem része az adatnak, így egy újraszerializálás elvben eldobhatja — a Lore-nak ezért a frontmatter-újraírásakor gondoskodnia kell a komment megőrzéséről. A valódi robusztusság (id-ütközés detektálása, frontmatter-validáció betöltéskor, duplikátum-feloldás) továbbra is **nyitott kérdés** (lásd lentebb).
...
A Lore ezt **nem fájlnév alapján**, hanem a frontmatter `id` mezője alapján oldja fel. Ez biztosítja, hogy a dokumentumok szabadon átnevezhetők vagy áthelyezhetők legyenek anélkül, hogy a hivatkozások eltörnének.

**Átnevezés / áthelyezés:** ha `docs/backend/payment-api.md` átkerül ide: `docs/payment/api.md`, a hivatkozások nem törnek el, mert a dokumentum identitását nem az útvonala, hanem az `id` határozza meg.
...
**Döntés:** a felhasználóknak **nincs** szükségük GitHub write-jogra a repóhoz. A pusht mindig a **Lore bot** végzi — ez így is a helyes: a repo integritása egy kézben marad, és a nem-technikai célközönség (PM, BA, UX) nem függ a repo-jogosultságoktól.
...
- A Git repo **all-or-nothing**: nincs per-fájl jogosultság. Akinek read-je van a repóhoz, az minden dokumentumot lát. Ha érzékeny (pl. stratégiai) doksik érzékenyebb hozzáférést igényelnek, azt repo-szeparációval kell megoldani, nem a Lore-on belül — ez enterprise kontextusban tudatosan kezelendő korlát.
...
| Publikálatlan piszkozat, élő session | Szerver | Átmeneti |
...
**Least-privilege engedély-kör.** A `/docs`-only működés miatt a GitHub App jogköre szűkre húzható: elég a **Contents** (read/write — az olvasáshoz, a `docs-sync` commitokhoz és pushhoz), a **Pull requests** (a `docs-sync` → `main` PR nyitásához) jog, valamint a **push webhook** (a külső változások detektálásához). Ez az enterprise beszerzésnél is erős érv: a Lore csak annyi hozzáférést kér, amennyi feltétlenül szükséges.

Fontos pontosítás a biztonsági review-hoz: a GitHub App engedélyei **repo-szinten** hatnak, nem könyvtár-szinten — a Contents-jog technikailag a teljes repóra vonatkozik, nem korlátozható pusztán a `/docs`-ra. A least-privilege itt tehát a **jogtípusok** szűkítését jelenti (nincs Administration, Actions, Secrets stb. hozzáférés), nem path-alapú korlátozást. A `/docs`-only megszorítás a Lore *saját* működésének önkorlátozása (csak azt olvassa/írja), nem a GitHub által kikényszerített határ.
...
**Döntés:** a Lore szerveroldali repo-tükröt (mirror) tart, **de kizárólag a `/docs` könyvtárra**. A szerver csak azt a részt tükrözi, amivel ténylegesen dolgozik. Ez gyors olvasást és helyben futó, egyszerű 3-way merge-et ad (a Publish-mechanikához kell a valódi commit-gráf), miközben a `/docs`-only szűkítés kis compliance-felületet, kis tárat és gyors syncet eredményez. A mirror a filozófia szerint **derivált, bármikor eldobható és újraépíthető** cache.
...
- **A `/docs`-on kívüli változásokra a mirror „vak".** Ez szándékos: a kódváltozások nem érintik a Lore-t, kivéve a PR merge-base számítását — amit a teljes commit-gráf megőrzése fed le.
...
- **MVP-indulás:** kereskedelmi API EU-régióval és a fenti szerződéses minimummal a pragmatikus start; a provider-absztrakció miatt EU-honos vagy on-prem modellre később fájdalommentesen át lehet állni, ha egy ügyfél megköveteli.
...
- **Jogosultság és szerzőség.** A Lore bot pushol (nincs szükség felhasználói write-jogra); a szerzőség a Git author-mezőn (`--author`) és a frontmatter `owner`-en keresztül marad a valódi emberé. Korlát: a repo all-or-nothing hozzáférésű. (Lásd: *Jogosultság és szerzőség*.)
